...
_create_state :: proc(allocator: runtime.Allocator) -> rawptr {
    return new(Web_State, allocator)
}
...
@(private = "file")
_cancel_close :: proc() {
    if _active != nil do _active.closed = false
}
...
_set_window_user_ptr :: proc(state: rawptr, ptr: rawptr) {
    ...
    js.add_window_event_listener(.Focus, ptr, _focus_callback)
    js.add_window_event_listener(.Blur, ptr, _focus_callback)
    s.listening = true
}
...
_remove_listeners :: proc(s: ^Web_State) {
    ...
    js.remove_window_event_listener(.Focus, ptr, _focus_callback)
    js.remove_window_event_listener(.Blur, ptr, _focus_callback)
    s.listening = false
    s.window = nil
}

@(private = "file")
_focus_callback :: proc(e: js.Event) {
    w := cast(^Window)(e.user_data)
    if w == nil do return
    _push_event(w, FOCUS_CHANGED{focused = e.kind == .Focus})
}
